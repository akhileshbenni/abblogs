...
export default function FeedItem({ post, currentUserId }: FeedItemProps) {
  const isLikedInitially = post.likes.some((l) => l.userId === currentUserId);
    ...
    <article className={styles.container}>
      ...
      <div className={styles.contentArea}>
        ...
        {post.mediaUrl && post.mediaType?.startsWith("image/") && (
          <img src={post.mediaUrl} alt="Post media" className={styles.mediaItem} />
        )}
        {post.mediaUrl && post.mediaType?.startsWith("video/") && (
          <video src={post.mediaUrl} controls className={styles.mediaItem} />
        )}
        <div className={styles.actions}>
          ...
          <button className={`${styles.actionBtn} ${styles.likeBtn} ${isLiked ? styles.liked : ""}`} onClick={handleLike}>
            ...
            <span className={styles.actionCount}>{likesCount > 0 ? likesCount : ""}</span>
          </button>
        </div>
      </div>
    </article>
